...
const makePostPersistenceReport = make.modules.find((module) => module.order === 16);
...
assert(make.scenarioSettings.processInOrder === true, 'Make must avoid overlapping executions for run checkpoints');
...
assert(/reverse the fetched page/i.test(makeListRuns.runWindow.sort), 'Make must reverse the fetched desc page before processing');
assert(/no checkpointed terminal run boundary/i.test(makeListRuns.runWindow.overflowGuard), 'Make must stop loudly on an overflowing uncheckpointed page');
assert(make.taskLaunch.requirements.pollRunLimit === 1000, 'Make run poll limit must remain 1000');
assert(make.taskLaunch.requirements.overflowStopIfNoCheckpointBoundary === true, 'Make must require an overflow stop');
...
assert(make.taskLaunch.requirements.preflightExistingCheckpointRead === true, 'Make must read pre-existing checkpoints before processing');
assert(make.taskLaunch.requirements.runOrdering === 'reverse-fetched-desc-page-before-processing', 'Make must define run ordering');
...
assert(make.modules.find((module) => module.label === 'Preflight Existing Run Checkpoints').order === 2, 'Make must read checkpoint keys in module 2');
assert(make.modules.find((module) => module.label === 'Preflight Overflow Guard').order === 3, 'Make must compute overflow in module 3');
assert(/must not reach module 5/.test(JSON.stringify(make.modules.find((module) => module.label === 'Overflow Guard Router'))), 'Make overflow route must stop before processing');
...
assert(makeDestination.configuration.key === 'rss:{{encodeURL(module 13 feedUrl)}}:{{encodeURL(module 13 itemKey)}}', 'Make RSS key must include encoded feedUrl and itemKey');
...
assert(makePostPersistenceReport.route === 'after module 14 when status != SUCCEEDED', 'Make RSS terminal report must follow the Data store write');
assert(/idempotently persisted/i.test(makePostPersistenceReport.purpose), 'Make RSS terminal report must prove persistence first');
assert(makeRunCheckpoint.idempotencyKey === 'rss:run:runId', 'Make run checkpoint must use run ID idempotency');
assert(/after all product row upserts/i.test(makeRunCheckpoint.checkpointInvariant), 'Make run checkpoint must be written last');
assert(makeRunCheckpoint.order === 19, 'Make run checkpoint must be module 19');
...
assert(/run checkpoint/i.test(make.publicationGate.join(' ')), 'Make account gate must verify run checkpointing');
assert(/desc=1&limit=1000&offset=0/i.test(make.publicationGate.join(' ')), 'Make account gate must verify the max poll page');
assert(/overflow-stop/i.test(make.publicationGate.join(' ')), 'Make account gate must verify overflow-stop behavior');

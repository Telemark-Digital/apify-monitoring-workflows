...
| 16 | Data store - Add/Replace a Record | Missing-dataset cursor. Write `bluesky:cursor:{{TASK_ID}}` only after module 15. |

Valid Bluesky records preserve `uri`, `url`, author handle/display name, text, createdAt, matchedTerms, match source, `isNew`, and metrics including quotes. Invalid rows are diagnostic records and do not manufacture missing identity fields.
...
2. Confirm the saved Task has `maxPostsPerRun <= 100`, matching the fixed non-paginated retrieval limit of 100, and the Apify Schedule interval is strictly longer than the Task hard timeout.

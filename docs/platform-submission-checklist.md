...
- Build the scenario from the structured Make specification using HTTP modules: list recent Task runs, read the exact Task cursor, fetch dataset items for cursor-selected terminal runs, write product rows, then write the run cursor last.
...
- Prime the exact run cursor before first activation, keep `maxRunsPerScenarioExecution = 1`, then configure the Make scenario schedule so expected outage coverage stays within the 1000-run polling window; if the expected outage window can exceed `1000 * Apify Schedule interval`, add paginated backfill before activation.
- Set **Store incomplete executions** to **Yes** and enable process-in-order/no-overlap behavior.
- Reproduce every module, filter, mapping, retrieval limit, diagnostic branch, Retry handler, and the module 11 Rollback/stop-on-error handler in the specification.
...
- Run the non-empty, quiet, invalid-record, terminal-with-committed-rows, duplicate-poll replay, failed-empty, missing-dataset, list-runs-retry, dataset-retry, cursor-read, run-cursor, completion-barrier, and data-store failure cases listed by the product package. Confirm status diagnostics follow persistence. Inject a module 11 post-commit timeout/failure and prove module 12/module 14 do not run in that execution, the run cursor remains unchanged, replay leaves exactly one stable product key, and a later successful run leaves exactly one cursor record.
...
- Verify the imported scenario still uses HTTP polling with `desc=1&limit=1000&offset=0`, exact cursor read/write, cursor-filtered reverse-page processing, one selected run per execution, and a loud stop when the fetched page has no cursor boundary, then repeat the full test matrix.
